...
class QLearningAgent(BustersAgent, ReinforcementAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        if len(self.getLegalActions(nextState)) == 0:
            self.qtable[qstate_actual][column] = (
                1 - self.alpha
            ) * qValue + self.alpha * (reward + 0)
        else:
            # Lo actualizamos
            self.qtable[qstate_actual][column] = (
                1 - self.alpha
            ) * qValue + self.alpha * (
                reward + self.discount * self.getValue(qstate_siguiente)
            )
    ...
    def final(self, state):
        ...
        print(
            "num_updates acumulado:  "
            + str(self.num_updates)
            + ", score final: "
    ...
    def modelo1(self, gameState, pacman_postition, posicion_fantasma):
        pacpos=pacman_postition
        ghostpos=posicion_fantasma
        state = ""
        # Miro lo que tengo a los lados
        for i in range(-1, 3, 2):
            if gameState.hasWall(pacpos[0] + i, pacpos[1]):
                state += "W"
            elif (pacpos[0] + i, pacpos[1]) in ghostpos:
                state += "G"
            else:
                state += "E"
            # Miro las casillas que tengo encima y debajo del lado al que me haya movido --> la diagonal al pacman
            for k in range(-1, 3, 2):
                if gameState.hasWall(pacpos[0] + i, pacpos[1] + k):
                    state += "W"
                elif (
                    pacpos[0] + i,
                    pacpos[1] + k,
        ...
        for j in range(-1, 3, 2):
            if gameState.hasWall(pacpos[0], pacpos[1] + j):
                state += "W"
            elif (pacpos[0], pacpos[1] + j) in ghostpos:
                state += "G"
            else:
                state += "E"
        state += ")"
        ...
        if any(x != None for x in gameState.data.ghostDistances):
            fantasma_index = gameState.data.ghostDistances.index(
            ...
            state += (
                ...
                + str(
                    distancer.getDistance(
                        pacpos, ghostpos[fantasma_index]
                    )
                )
                + ")"
            )
    ...
    def modelo2(self, gameState, pacman_position, posicion_fantasma):
        pacpos = pacman_position
        ghostpos = posicion_fantasma
        state = ""
        # Modelo 2Direccion del fantasma mas cercano y lo que tengo alrededor en la 4 casillas de direccion
        # Miro la direccion del fantasma mas cercano
        
        if any(x != None for x in gameState.data.ghostDistances):
            fantasma_index = gameState.data.ghostDistances.index(min(x for x in gameState.data.ghostDistances if x is not None))
            distancia_x= pacpos[0]-ghostpos[fantasma_index][0]
            distancia_y= pacpos[1]-ghostpos[fantasma_index][1]
            if (abs(distancia_x)>=abs(distancia_y)):
                if (distancia_x>0):
                    state +='L'
                if (distancia_x<0):
                    state +='R'
                    #cuidado no contemplo = 0

            elif(abs(distancia_x)<abs(distancia_y)):
                if (distancia_y>0):
                    state +='D'
                if (distancia_y<0):
                    state +='U'
                ...
                distancer = Distancer(gameState.data.layout)
                state += '(' + str(distancer.getDistance(pacpos, ghostpos[fantasma_index])) + ')'
            #Miro los lados
            for i in range (-1, 3, 2):
                if gameState.hasWall(pacpos[0] + i, pacpos[1]):
                    state += 'W'
    ...
    # Modelo 3
    def modelo3(self, gameState, pacman_position, posicion_fantasmas):
        # Miro lo que tengo a los lados
        state = ""
        if any(x != None for x in gameState.data.ghostDistances):
            fantasma_index = gameState.data.ghostDistances.index(min(x
                for x in gameState.data.ghostDistances if x
                is not None))
            distancia_x = pacman_position[0] \
                - posicion_fantasmas[fantasma_index][0]
            distancia_y = pacman_position[1] \
                - posicion_fantasmas[fantasma_index][1]
            if abs(distancia_x) >= abs(distancia_y):
                if distancia_x > 0:
                    state += 'L'
            ...
            distancer = Distancer(gameState.data.layout)
            state += '(' + str(distancer.getDistance(pacman_position,
                            posicion_fantasmas[fantasma_index])) \
                + ')'
        for i in range(-1, 3, 2):
            if gameState.hasWall(pacman_position[0] + i,
                                pacman_position[1]):
                state += 'W'
            elif (pacman_position[0] + i, pacman_position[1]) \
                in posicion_fantasmas:
                state += 'G'
            else:
                state += 'E'
            ...
            for k in range(-1, 3, 2):
                if gameState.hasWall(pacman_position[0] + i,
                                    pacman_position[1] + k):
                    state += 'W'
                elif (pacman_position[0] + i, pacman_position[1] + k) \
                    in posicion_fantasmas:
                    state += 'G'
                else:
                    state += 'E'
        ...
        for j in range(-1, 3, 2):
            if gameState.hasWall(pacman_position[0], pacman_position[1]
                                + j):
                state += 'W'
            elif (pacman_position[0], pacman_position[1] + j) \
                in posicion_fantasmas:
                state += 'G'
            else:
                state += 'E'
    ...
    def modelo4(self, gameState, pacman_position, posicion_fantasmas):
        state = ""
        for i in range (-1, 3, 2):
    ...
    # def modelo5(self, gameState, pacman_position, posicion_fantasmas):
    #     print(pacman_position)
    #     return""
    def modelo5(self, gameState, pacman_position, posicion_fantasmas):
        pacpos = pacman_position
        ghostpos = posicion_fantasmas
        state = ""
        # Modelo 2Direccion del fantasma mas cercano y lo que tengo alrededor en la 4 casillas de direccion
        # Miro la direccion del fantasma mas cercano
    ...
    def modelotest (self, gameState, pacman_position, posicion_fantasmas):
            ...
            for j in range(-1,2,1):
                ...
                try:
                    if gameState.hasWall(pacman_position[0] + i, pacman_position[1] + j):
                        state += 'W'
                    elif (pacman_position[0] + i, pacman_position[1] + j) in posicion_fantasmas:
                        state += 'G'
        ...
        state += ')'
        if any(x != None for x in gameState.data.ghostDistances):
            ghost_index = gameState.data.ghostDistances.index(
                min(x for x in gameState.data.ghostDistances if x is not None)
            )
            diff_x, diff_y = (
                p - g for p, g in zip(posicion_fantasmas[ghost_index], pacman_position)
            )
            if diff_y > 0:
                if diff_x > 0:
                    state += "(UR)"
            ...
            distancer = Distancer(gameState.data.layout)
            state += "(" + str(
                distancer.getDistance(pacman_position, posicion_fantasmas[ghost_index])
            )
        state += ")(" + (
            "last"
            if len([living for living in gameState.getLivingGhosts() if living is True])
            == 1
            else "more"
        )
